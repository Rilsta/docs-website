...
redirects:
  ...
  - /docs/log-management/new-relic-logs/enable-logs/enable-new-relic-logs-fluent-bit
  - /docs/log-management/enable-logs/enable-logs/fluent-bit-plugin-logs
  - /docs/log-management/enable-log-monitoring-new-relic/enable-log-monitoring-new-relic/fluent-bit-plugin-log-forwarding
---
...
Fluent Bit needs to know the location of the New Relic plugin and the [New Relic license key](/docs/accounts/install-new-relic/account-setup/license-key) **(recommended)** or [Insert API key](/docs/insights/insights-data-sources/custom-data/send-custom-events-event-api#register) to output data to New Relic. To configure your Fluent Bit plugin:
...
3. In the `fluent-bit.conf` file, add the following line under the `service` block:
   ...
   ```
4. At the bottom of the `fluent-bit.conf` file, add the following to set up the input and output filters. Replace the placeholder text with your [New Relic license key](/docs/accounts/install-new-relic/account-setup/license-key) **(recommended)** or [Insert API key](/docs/insights/insights-data-sources/custom-data/send-custom-events-event-api#register):
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        New Relic's [Insert API key](/docs/insights/insights-data-sources/custom-data/send-custom-events-event-api#register). Use either `licenseKey` (**recommended**) or `apiKey`, not both.
      </td>
    </tr>
  </tbody>
</table>
