...
6. Restart the `rsyslog` service by running:
   ...
   ```
7. [Check your New Relic account](/docs/log-management/log-management/ui-data/explore-your-data-log-analytics) for logs.
...
4. Optional: Configure `syslog-ng` to tail files by adding the following to the `Sources` configuration block:
   ...
   ```
5. Define the New Relic `syslog` format and add your New Relic [Insights API Insert key](/docs/telemetry-data-platform/ingest-manage-data/ingest-apis/use-event-api-report-custom-events#register):

   ```
   template NRFormat { template("<var>YOUR_INSERT_KEY</var> ${PRI}>1 ${ISODATE} ${HOST:--} ${PROGRAM:--} ${PID:--} ${MSGID:--} ${SDATA:--} $MSG\n");
     template_escape(no);
   };
   ```
...
8. Restart `syslog-ng` by running:
   ...
   ```
9. [Check your New Relic account](/docs/log-management/log-management/ui-data/explore-your-data-log-analytics) for logs.
