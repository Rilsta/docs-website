...
redirects:
  - /docs/log-management/new-relic-logs/apm-integration/apm-agent-apis-manual-logs-context-instrumentation
  - /docs/log-management/new-relic-logs/apm-integration/decorating-logs-logs-context-using-apm-agent-apis
  - /docs/log-management/new-relic-logs/apm-integration/annotating-logs-logs-context-using-apm-agent-apis
  - /docs/log-management/new-relic-logs/apm-integration/annotate-logs-logs-context-using-apm-agent-apis
  - /docs/log-management/enable-logs/logs-context-agent-apis/annotate-logs-logs-context-using-apm-agent-apis
  - /docs/log-management/enable-log-monitoring-new-relic/logs-context-agent-apis/annotate-logs-logs-context-using-apm-agent-apis
---

If you use log management and APM, you can configure [logs in context](/docs/log-management/new-relic-logs/enable-logs-context/enable-logs-context-apm-agents) to correlate log data with other New Relic features.

If you use a logging framework not covered by our existing [logs in context solutions](/docs/log-management/new-relic-logs/enable-logs-context), we provide a way for you to configure your logging libraries to achieve annotated logs necessary for logs in context.

## APM agent trace metadata and linking metadata APIs [#apis]

To get properly annotated logs for [logs in context](/docs/log-management/new-relic-logs/enable-logs-context/enable-logs-context-apm-agents), use APM agent API calls (listed below). These APIs pass metadata required for linking log data to other New Relic data. This metadata is referred to as **trace metadata** and **linking metadata**. For tips on using these, see [Resources for correct annotation](#tips).
...
* View the source code for our own logs in context extensions to see how we use these APIs:
  ...
  * Python: see [Python agent logging configuration](/docs/log-management/new-relic-logs/enable-logs-context/enable-logs-context-python#configure-logging)
