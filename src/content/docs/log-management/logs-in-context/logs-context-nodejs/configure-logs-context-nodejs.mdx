---
title: Configure logs in context for Node.js
tags:
  - Logs
  - Enable log management in New Relic
  - Logs in context for Node.js
metaDescription: A description of the New Relic Logs logs in context configuration process for the Node.js agent.
redirects:
  - /docs/log-management/new-relic-logs/enable-logs-context/enable-logs-context-nodejs
  - /docs/log-management/enable-logs/logs-context-node/configure-logs-context-node
  - /docs/log-management/enable-log-monitoring-new-relic/logs-context-nodejs/configure-logs-context-nodejs
---

Logs in context for the Node.js agent connects your Logs and APM data in New Relic, giving full context to high-level events and providing high value data to specific log lines.

## Compatibility and requirements [#compatibility-requirements]

To use log management with the Node.js agent, ensure your configuration meets the following requirements:

* [Log management](/docs/introduction-new-relic-logs) enabled, with a compatible [log forwarding plugin](/docs/log-management/enable-logs/enable-logs/enable-new-relic-logs#enable-logs) installed
* Node.js agent 6.2.0 or higher: [Install](/docs/agents/nodejs-agent/installation-configuration/install-nodejs-agent) or [update](/docs/agents/nodejs-agent/installation-configuration/update-nodejs-agent)

## Configure logs in context for Node.js [#configure-logs-java]

Choose a extension to see specific instructions:

* [Winston](/docs/log-management/new-relic-logs/enable-logs-context/enable-logs-context-winston)

## What's next? [#what-next]

Now that you've set up APM logs in context, here are some potential next steps:

* Explore your data using the [Logs UI](/docs/explore-your-data-new-relic-logs-ui).
* Troubleshoot errors with [distributed tracing](/docs/understand-dependencies/distributed-tracing/get-started/introduction-distributed-tracing), stack traces, application logs, and more.
* [Query your data](/docs/using-new-relic/data/understand-data/query-new-relic-data) and [create custom dashboards](/docs/dashboards/new-relic-one-dashboards/get-started/introduction-new-relic-one-dashboards) or [alerts](/docs/alerts/new-relic-alerts/configuring-alert-policies/create-edit-or-find-alert-policy).
